...
from openpilot.selfdrive.mads.helpers import MadsSteeringModeOnBrake
...
class ModularAssistiveDrivingSystem:
  def __init__(self, selfdrive):
    ...
    self.steering_mode_on_brake = self.params.get("MadsSteeringMode", return_default=True)
  ...
  def data_sample(self):
    ...
    # we want to disengage. The status from the panda goes through another socket
    # than the CAN messages, so allow a mismatch for two samples.
  ...
  def update_events(self, CS: structs.CarState):
    ...
    for be in CS.buttonEvents:
      ...
      if be.type == ButtonType.lkas and be.pressed and CS.cruiseState.available:
        if self.enabled:
