...
TogglesPanel::TogglesPanel(QWidget *parent) : QWidget(parent) {
  ...
  if (Hardware::TICI()) {
    toggles.append(new ParamControl("EnableWideCamera",
                                    "広角カメラの使用を可能にする",
                                    "運転やUIに広角カメラを使用する。再起動後にのみ反映されます。",
                                    "../assets/offroad/icon_openpilot.png",
                                    this));
...
void DeveloperPanel::showEvent(QShowEvent *event) {
  ...
  QList<QPair<QString, std::string>> dev_params = {
    {"バージョン", brand + " v" + params.get("Version", false).substr(0, 14)},
    {"Gitブランチ", params.get("GitBranch", false)},
    {"Gitコミット", params.get("GitCommit", false).substr(0, 10)},
    {"パンダファームウェア", params.get("PandaFirmwareHex", false)},
